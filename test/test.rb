...
=end
  ...
  def test_sqlstate()
    if @m.server_version >= 40100 then
      ...
      assert_raises(Mysql::ParseError){@m.query("hogehoge")}
...
@m.charset = "ascii"
  ...
  def test_fetch_field_direct()
    ...
    assert_equal("str", f.name)
    assert_raises(Mysql::ClientError){@res.fetch_field_direct(-1)}
    assert_raises(Mysql::ClientError){@res.fetch_field_direct(2)}
  end
...
=end
  ...
  def test_execute3()
    if @m.server_version >= 40100 then
      ...
      assert_raises(Mysql::ClientError){@s.execute(123, "hoge")}
      assert_raises(Mysql::ClientError){@s.execute(123, "hoge", 0, "fuga")}
      @s.prepare("select * from t")
      @s.execute
...
=end
  ...
  def test_prepare()
    if @m.server_version >= 40100 then
      @s.prepare("select 1")
      assert_raises(Mysql::ParseError){@s.prepare("invalid syntax")}
    end
  end
...
=end
  ...
  def test_sqlstate()
    if @m.server_version >= 40100 then
      ...
      assert_raises(Mysql::ParseError){@s.prepare("hogehoge")}
